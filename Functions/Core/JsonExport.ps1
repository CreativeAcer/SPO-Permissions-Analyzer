...
function Export-DataTypeJson {
    <#
    .SYNOPSIS
    Exports a specific data type as JSON
    #>
    param(
        [Parameter(Mandatory = $true)]
        [ValidateSet("sites", "users", "groups", "roleassignments", "inheritance", "sharinglinks", "all")]
        [string]$DataType,
        [string]$OutputPath
    )
    ...
    if ($DataType -eq "all") {
        return Export-GovernanceJson -OutputPath $OutputPath
    }
    ...
    $mappedType = $typeMap[$DataType.ToLower()]
    $data = Get-SharePointData -DataType $mappedType

    $filePath = Join-Path $OutputPath "spo_${DataType}_$timestamp.json"
    ...
    $output | ConvertTo-Json -Depth 10 | Set-Content $filePath -Encoding UTF8

    Write-ActivityLog "JSON export ($DataType): $filePath" -Level "Information"
    return $filePath
}
